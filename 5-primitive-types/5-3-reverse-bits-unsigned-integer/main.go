...
func brutereverse(n uint64) uint64 {

	bits := []byte(fmt.Sprintf("%08s", strconv.FormatUint(n, 2)))
	l := len(bits)
	...
	reversed, _ := strconv.ParseUint(string(bits), 2, 64)
...
func buildLookupTable() {
	var table = []uint8{}
	for i := 0; i < 256; i++ {
		table = append(table, uint8(divreverse(uint64(i))))
	}
	fmt.Printf("%#v", table)
}
...
func main() {
	//buildLookupTable()
	fmt.Printf("%s\n%s\n%s\n%s", strconv.FormatUint(123456789, 2),
		strconv.FormatUint(divreverse(123456789), 2),
		strconv.FormatUint(brutereverse(123456789), 2),
		strconv.FormatUint(lookupreverse(123456789), 2))
